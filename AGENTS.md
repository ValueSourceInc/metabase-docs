...
This file tells AI coding agents how to use this knowledge base when working on Metabase tasks.
It contains NO specific data — only reading strategies. All data lives in the generated docs.

## What This Project Is

A generated documentation library of the Metabase instance. Run `pnpm gen` to refresh from the Metabase API. For current stats, see `_index.json` → `.summary`.
...
5. **Prefer the Metabase search API for name lookup when available** — server-side search costs zero local tokens; fall back to `_catalog.md` only when API is unavailable or you need the full domain/collection view.
