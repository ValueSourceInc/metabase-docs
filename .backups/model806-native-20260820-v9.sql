...
v_us AS (
  SELECT pm.store, m2.sku,
         ...
         m2."afn-inbound-working-quantity"::int AS ibw, m2."afn-inbound-shipped-quantity"::int AS ibs,
         m2."afn-inbound-receiving-quantity"::int AS ibr
...
j AS (
  SELECT m.id,
    CASE WHEN p.scope <> 'CA' THEN 'US_MARKETPLACE'
         ...
         WHEN COALESCE(n.st,'') IN ('Enabled','Action required') THEN
           CASE WHEN COALESCE(l.e,0) > 0 THEN 'LOCAL_STOCK'
                ...
                ELSE 'MIRROR_LAG' END
         -- remote 显示 Not eligible 但并非"本地有货互斥":加美仓库量/可售量完全相等且台账无新鲜结存,
         -- 且美国侧有货(排除 0==0 空洞相等)-> 实为镜像(实证:14 个 DH-BT-LEAF,台账 8 个月零行,
         -- CA 在途 = US 镜像 + 真实 24 件/SKU,业务出货表可对账)。按镜像修正。
         WHEN n.st = 'Not eligible'
              AND COALESCE(l.e,0) = 0
              AND (u.whq > 0 OR u.ffq > 0)
              AND m."afn-warehouse-quantity"::int = u.whq
              AND m."afn-fulfillable-quantity"::int = u.ffq THEN 'MIRROR_NOT_ELIGIBLE'
    ...
    u.ibw AS uibw, u.ibs AS uibs, u.ibr AS uibr
...
dec AS (
  SELECT id, flag, lede,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cwhq-uwhq,0)
         WHEN flag='LOCAL_STOCK' THEN LEAST(cwhq,lede) ELSE cwhq END AS a_whq,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cffq-uffq,0)
         WHEN flag='LOCAL_STOCK' THEN LEAST(cffq,lede) ELSE cffq END AS a_ffq,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cusq-uusq,0) ELSE cusq END AS a_usq,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(crsq-ursq,0) ELSE crsq END AS a_rsq,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(crsrch-ursrch,0) ELSE crsrch END AS a_rsrch,
    -- 在途列用减法而非清零:CA 行在途 = 美国镜像部分 + 真实发往 CA 的在途(实证 2026-08-20:
    -- 118 个镜像行中 116 个 CA−US 差值与业务出货表 wps_shipment_items 的 CA 未签收在途完全相等,
    -- 如 QS001-K-ST-PK 456−282=174=在途出货 156+18;台账也证实镜像 SKU 持续有 CA 收货)。
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cibw-uibw,0) ELSE cibw END AS a_ibw,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cibs-uibs,0) ELSE cibs END AS a_ibs,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cibr-uibr,0) ELSE cibr END AS a_ibr,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cfct-ufct,0) ELSE cfct END AS a_fct,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cohb-uohb,0) ELSE cohb END AS a_ohb,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(crfs-urfs,0) ELSE crfs END AS a_rfs,
    CASE WHEN flag IN ('MIRROR','MIRROR_LAG','MIRROR_NOT_ELIGIBLE') THEN GREATEST(cfsb-ufsb,0) ELSE cfsb END AS a_fsb
  FROM j
)
SELECT
